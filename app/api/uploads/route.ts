...
export async function POST(req: Request) {
	try {
		...
		const stored = await adapter.save(buffer, {
		...
		if (!validated.success) {
			return NextResponse.json({ error: "Validation failed", issues: validated.error.flatten() }, { status: 400 });
		}
		...
		return NextResponse.json(
			{ error: "Upload failed", details: e?.message ?? String(e) },
			{ status: 500 }
		);
	}
}
